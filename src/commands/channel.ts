...
export default class ChannelCommand implements Command {
    ...
    async run(message: Message, [ curr, ...arg ]: string[], { use, mustBeVoice }: Messages): Promise<any> {
        ...
        if(!channel) message.reply(`${use} \`${this.info.usage}\``);
        if(channel.type != 'voice') message.reply(mustBeVoice);
