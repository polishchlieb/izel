        ...
        <div class="tab">
            ...
            <div class="tab-content">
                <div class="row">
                    ...
                    <div class="button" @click="popup('Autorole', 0, settings.roles)">{{ $root.$data.strings.change }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="tab">
            ...
            <div class="tab-content">
                ...
                <div class="roleCat" v-for="(cat, i) in settings.selfroles" :key="i">
                    ...
                    <div class="button" @click="popup($root.$data.strings.addRole, 2, {roles: settings.roles, cat: cat.name})">{{ $root.$data.strings.addRole }}</div>
                    <div class="button" @click="removeCat(cat.name)">{{ $root.$data.strings.removeCat }}</div>
                </div>
                <div class="button" @click="popup($root.$data.strings.addRole, 3)">{{ $root.$data.strings.addCat }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        popupReturn: function(type, data) {
            if(type == 0) { // autorole 
                fetch('/api/admin', {
	                ...
	                body: JSON.stringify({guild: this.$props.guild.id, action: {id: "autorole"}, value: data.id})
                })
                .then(resp => {
                    if(resp.ok) {
...
</style>https://stackoverflow.com/questions/10024866/remove-object-from-array-using-javascript